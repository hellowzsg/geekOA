...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'view'], function(exports){
    "use strict";
    ...
    Class.prototype.pullData = function(curr){
      ...
      if(options.url){ //Ajax请求        
        ...
        if(setter.request.tokenName){
            //自动给参数传入默认 token
            params[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');            
            //自动给 Request Headers 传入 token
            options.headers = options.headers || {}
            options.headers[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');            
        }
        ...
        var ext = options.url.substr(options.url.lastIndexOf(".") + 1);        
        $.ajax({
          type: options.method || ( (ext == 'js' || ext == 'json') ? 'get' : 'post')
          ,url: options.url
          ...
          ,success: function(res){
            ...
            if(res[setter.response.statusName] == statusCode.logout){
                view.exit();
            } else if(res[setter.response.statusName] != statusCode.ok) {
                var error = [
                    '<cite>Error：</cite> ' + (res[setter.response.msgName] || '返回状态码异常')
                    ,debug()
                  ].join('');
                  view.error(error);
            } else {
