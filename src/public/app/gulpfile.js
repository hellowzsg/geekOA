...
,task = {
  ...
  ,mv: function(){    
    ...
    gulp.src('./src/style/extend/**/*.eot')
    .pipe(gulp.dest(destDir + '/style/extend'));
    gulp.src('./src/style/extend/**/*.svg')
    .pipe(gulp.dest(destDir + '/style/extend'));
    gulp.src('./src/style/extend/**/*.ttf')
    .pipe(gulp.dest(destDir + '/style/extend'));
    gulp.src('./src/style/extend/**/*.woff')
    .pipe(gulp.dest(destDir + '/style/extend'));
    gulp.src('./src/style/extend/**/*.json')
    .pipe(gulp.dest(destDir + '/style/extend'));
...
//构建核心源文件
gulp.task('default', gulp.series('clear', 'mincss', 'minjs', 'mv'), function(){ //命令：gulp
  for(var key in task){
    task[key]();
  }
});