...
//			level: "level",			//层级
	...
	DTree.prototype.init = function(){
		...
		if(_this.data) {
			...
			_this.params = _this.getFilterRequestParam(_this.getRequestParam());
			var setter = layui.setter;
			if(setter.request.tokenName) {
				//自动给参数传入默认 token
				_this.params[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');
			}

			//自动给 Request Headers 传入 token
			_this.headers = _this.headers || {}
			_this.headers[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');
