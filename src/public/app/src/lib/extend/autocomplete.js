layui.define(['jquery', 'laytpl', 'layer', 'view'], function (e) {
    "use strict";
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
    ...
    job.prototype.pullData = function () {
        ...
        if(setter.request.tokenName) {
            //自动给参数传入默认 token
            _config.params[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');
        }

        //自动给 Request Headers 传入 token
        _config.headers = _config.headers || {}
        _config.headers[setter.request.tokenName] = (layui.data(setter.tableName)[setter.request.tokenName] || '');
